...
return function()
	describe("Eva invoices", function()
		...
		it("Should correct consume invoices", function()
			...
			assert(eva.tokens.get("money") == 0)
			assert(eva.tokens.get("exp") == 0)


			assert(eva.invoices.can_consume(id) == true)
			eva.invoices.consume(id)
			assert(events[CONSUME].calls == 1)
			assert(eva.tokens.get("money") == 10)
			assert(eva.tokens.get("exp") == 100)
			...
			assert(eva.tokens.get("money") == 20)
			assert(eva.tokens.get("exp") == 200)
		end)
