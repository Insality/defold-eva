...
local function is_can_schedule(push_time, category)
	...
	for i = 1, #pushes do
		...
		if push_code == code then
			...
			if push.category == category then
...
local function clear_old_pushes()
	local pushes = app[const.EVA.PUSH].pushes

	local cur_day_code = get_time_code()

	for i = #pushes, 1, -1 do
		local push_code = get_time_code(os.time("*t"), pushes[i].time)
...
function M.schedule(seconds_after, title, text, category)
	...
	local id = push_schedule(delta_time, title, text)
	...
	if id then
		...
		logger:info("Schedule local notification", {
			id = id,
			time = push_time,
...
function M.unschedule(id)
	...
	logger:info("Unschedule push", { id = id, category = push.category })
end
...
function M.on_eva_second()
	...
	for i = #pushes, 1, -1 do
		local push = pushes[i]
		if push.time < (current_time + time_gap) then
			M.unschedule(push.id)
		end
	end
end
